...
eruption_counts <- eruptions %>%
    mutate(
        ...
        start_date = as.Date(with(
            eruptions, paste(start_year, start_month, start_day, sep = "-")
        ), "%Y-%m-%d"),
        end_date = as.Date(with(
            eruptions, paste(end_year, end_month, end_day, sep = '-')
        ), '%Y-%m-%d'),
    ...
    summarize(number_of_eruptions = n(),
