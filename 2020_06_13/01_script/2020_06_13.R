...
# Pick a color palette for category variable
colors_palette <- c('#FFBC41','#067891','#A9AAB0','#EAEAEA','#BBACA3','#F2D6AE','#7C7D3D','#DF811D')

# Choose which firsts I want to add labels for
...
firsts_plot <- firsts %>% 
    ...
    geom_textbox(data = subset_rows, aes(label = label, x =  year,y = position),
              ...
              width = grid::unit(0.15, "npc"),
                 show.legend = FALSE) +
    # Change the color of the category variable
    ...
    # Set limits for the plot to ensure all labels show
    ylim(-115,115)+
    xlim(1720,2030) +
